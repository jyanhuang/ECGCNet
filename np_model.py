...
def Linear(input_data, outnumber, weights, bias):
    output = np.zeros(outnumber)
    for i in range(weights.shape[0]):
        for j in range(len(input_data)):
            output[i] += input_data[j] * weights[i, j]
        output[i] += bias[i]
...
def npConv_one_filter1D(img, shape, weight_one_filter, bias, kernel_size=3, isexpand=None):
    # 获取输入的维度信息
    length = shape[0]
    padding = int((kernel_size - 1) / 2) if int((kernel_size - 1) / 2) > 0 else 0
    result = [0] * length
    ...
    for i in range(length):
        ...
        for m in range(kernel_size):
            ...
            input_index = i + m
            if isexpand != None:
                output_value += (weight_one_filter[weight_index] * img[input_index]) >> 16
            else:
                output_value += (weight_one_filter[weight_index] * img[input_index]) >> 8
            findextreme(output_value)
            result[i] = output_value + (bias[0] >> 8)
        findextreme(result[i])
    return result


def npConv1D(feature, shape, weight, bias, in_channels=1, out_channels=1, kernel_size=3, padding=0, group=None, isexpand=None):
    length = shape[0]
    result = np.zeros(out_channels * length, dtype=int)
    if padding != 0:
    ...
    if group is None:
        for i in range(out_channels):
            for j in range(in_channels):
                ...
                result[i * length:(i + 1) * length] += np.array(npConv_one_filter1D(feature[j * (length + 2 * padding):(j + 1) * (length + 2 * padding)], shape, weight_one_filter, bias, kernel_size=kernel_size, isexpand=isexpand))
    else:
        for i in range(in_channels):
            weight_one_filter = weight[i * kernel_size:(i + 1) * kernel_size]
            result[i * length:(i + 1) * length] = npConv_one_filter1D(feature[i * (length + 2 * padding):(i + 1) * (length + 2 * padding)], shape, weight_one_filter, bias, kernel_size=kernel_size, isexpand=isexpand)
...
def npAvgPool1D(img, kernel_size, stride, padding=0, inchannel=1):
    ...
    for i in range(inchannel):
        ...
        for j in range(0, out_length):
            ...
            result[out_length * i + j] = int(sum(window) / kernel_size)
...
def np_nn(img, conv1_weight, conv1_bias, conv2_weight, conv2_bias, conv3_weight, conv3_bias, conv4_weight, conv4_bias, conv5_weight, conv5_bias, fc1_weight, fc1_bias, fc2_weight, fc2_bias): # 是一个神经网络的前向传播函数，用于对输入图像img进行处理，并返回最终的预测结果。
    conv1 = npConv1D(img, [300], conv1_weight, conv1_bias, 1, 1, 15, 7, isexpand=0)
    ...
    fc1 = Linear(conv5, 16, fc1_weight, fc1_bias)
    fc1 = [x if x >= 0 else 0 for x in fc1]
    fc2 = Linear(fc1, 5, fc2_weight, fc2_bias)
