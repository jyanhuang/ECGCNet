...
def main():
    ...
    end_time = time.time()  # recoding ending time

    inference_time = (end_time - start_time)/num_samples  # computing inference time

    throughput = num_samples / (end_time - start_time)
    average_latency = (end_time - start_time) / num_samples

    # Put the predicted results on CPU, and convert them to 'Numpy' array
    ...
    # Before drawing the confusion matrix heatmap, ensure y_true and y_pred are Numpy arrays or Lists
    y_true = y_test  # Do not use .cpu() method，directly use Numpy array
    y_pred = np.array(y_pred)  # Converting the predict label to Numpy array
    ...
    # Print the results
    print(f'delay: {average_latency:.5f} seconds per sample')
    print(f'throughput: {throughput:.5f} samples per second')
